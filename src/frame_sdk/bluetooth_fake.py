...
from .fake_bleak_client import FakeBleakClient, FakeGattCharacteristic
...
class Bluetooth:
    """
    Fake Frame bluetooth class that uses queues for messaging instead of actual
    ...
    def __init__(self):
        self._btle_client: Optional[FakeBleakClient] = None
    ...
    async def connect(
        ...
        """
        Connects to a fake Frame device using queues for message passing.
        ...
        device_address = "FF:FF:FF:FF:FF:FF"  # Fake address
        self._btle_client = FakeBleakClient(
            device_address,
            disconnected_callback=self._disconnect_handler,
        )

        # Set up the fake services and characteristics
        self._btle_client.add_service(
            self._SERVICE_UUID,
            {
                self._TX_CHARACTERISTIC_UUID: "tx",
                self._RX_CHARACTERISTIC_UUID: "rx"
            }
        )
        ...
        service = self._btle_client.services.get_service(
            self._SERVICE_UUID,
        )
    ...
    async def disconnect(self) -> None:
        """
        Disconnects from the fake device.
        """
    ...
    async def simulate_receive(self, data: bytearray) -> None:
        """
        Simulate receiving data from the fake device.
        This method allows tests to push data as if it came from the device.
        
        Args:
            data (bytearray): The data to simulate receiving from the device.
        """
        if not self.is_connected():
            raise Exception("Not connected")
        
        await self._btle_client.simulate_receive(self._RX_CHARACTERISTIC_UUID, data)
